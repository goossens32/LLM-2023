...
function ex1Filter(products){
  ...
  for (const product of products){  
    if (!productEx || product.price > productEx.price){
      productEx = product;
    }
  };
  return productEx;
...
function ex2Filter(products){
  let productCh = 0;
  for (const product of products){  
    if (!productCh || product.price < productCh.price){
      productCh = product;
    }
  };
  return productCh;
...
function ex3Filter(products){
  ...
  for (const product of products) {
    if (product.price > 100 && product.price < 500 && product.category === "ELEC"){
...
function ex4Filter(products){
  let productsFiltered = [];
  for (const product of products){
    if  ((product.price > 500 && product.price < 600) && 
...
function ex5Filter(products){
  let productsFiltered = [];
  for (const product of products){
    if (product.stock && product.name.toUpperCase().includes("GAMING")){
      productsFiltered.push(product);
    }
  }
  return productsFiltered;
...
function ex6Filter(products){
  for (const product of products){
    if (product.brand.toUpperCase() === "SAMSUNG" && product.category.toUpperCase() === "MOBILE"){
      return true
    }
    return false
  }
